...
func parsePool(path string, pool string) (*cfg.Config, error) {
	...
	if len(poolPart) > 0 { // the first one must be application name
		appconfig, err = cfg.NewConfigFromFile(path + applicationPath + poolPart[0] + fileSuffix)
	...
	// pool
	if len(poolPart) > 1 { // step loading
		base := poolPart[0]
		for _, v := range poolPart[1:] {
			base = base + poolNameSaperator + v
